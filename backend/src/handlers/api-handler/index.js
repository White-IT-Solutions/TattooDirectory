  ...
  try {
    ...
    if (config.isLocal) {
      ...
      try {
        ...
        const hasValidHits = result.body && result.body.hits && result.body.hits.hits;
        if (!hasValidHits) {
          logger.warn("OpenSearch returned empty or malformed response for get artist, falling back to DynamoDB", {
            indexName: getIndexName(),
            responseBody: result.body,
          });
          ...
          try {
            ...
            logger.error("DynamoDB fallback also failed for get artist", {
              error: dynamoError.toString(),
              artistId,
            });
            return createErrorResponse(
  ...
  try {
    ...
    if (config.isLocal) {
      ...
      try {
        ...
        // Check if OpenSearch returned valid aggregations
        const hasValidAggregations = result.body && result.body.aggregations && result.body.aggregations.unique_styles;
        if (!hasValidAggregations) {
          logger.warn("OpenSearch returned empty or malformed response for styles, falling back to DynamoDB", {
            indexName: getIndexName(),
            responseBody: result.body,
          });
...
export const handler = async (event, context) => {
  ...
  // Ensure logger is available, create fallback if needed
  if (!logger) {
    logger = createLogger(context, event);
  }
