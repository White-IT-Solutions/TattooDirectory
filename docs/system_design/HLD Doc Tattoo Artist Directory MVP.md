...
**GSI1 (Style-Location Index) Key Format:**

**gsi1pk (GSI1 Partition Key):**

- **Format:** `STYLE#{styleName}`
- **Style Normalization:** All style names are converted to lowercase and standardized
- **Example:** `STYLE#neotraditional`, `STYLE#blackwork`, `STYLE#realism`

**gsi1sk (GSI1 Sort Key):**

- **Format:** `LOCATION#{country}#{city}#{artistId}`
- **Location Normalization:** Country and city names are converted to uppercase for consistency
- **Example:** `LOCATION#UK#LEEDS#artist-123`, `LOCATION#UK#MANCHESTER#artist-456`

**Multiple Style Handling:**
When an artist specializes in multiple styles, the Fargate task creates **multiple GSI entries** for the same artist:

- Artist with styles ["neotraditional", "blackwork"] creates two GSI entries:
  - `gsi1pk: STYLE#neotraditional, gsi1sk: LOCATION#UK#LEEDS#artist-123`
  - `gsi1pk: STYLE#blackwork, gsi1sk: LOCATION#UK#LEEDS#artist-123`

**Important Implementation Note:**
The Fargate scraper task must create separate DynamoDB items for each style-location combination to properly populate the GSI. This means one artist with multiple styles will have multiple entries in the GSI, all pointing to the same base artist record.

**GSI2 (Instagram Index) Key Format:**

**gsi2pk (GSI2 Partition Key):**

- **Format:** `INSTAGRAM#{instagramHandle}`
- **Handle Normalization:** Instagram handles are stored without the '@' symbol, lowercase
- **Example:** `INSTAGRAM#alextattoo`, `INSTAGRAM#inkmaster_uk`

This design enables efficient queries such as:

- Find all neo-traditional artists in Leeds: `gsi1pk = STYLE#neotraditional AND begins_with(gsi1sk, 'LOCATION#UK#LEEDS')`
- Find artist by Instagram handle: `gsi2pk = INSTAGRAM#alextattoo`

**DynamoDB Table Configuration Alignment:**
The serverless.yml configuration defines the table structure with:

- **EntityIndex (GSI1):** Uses `gsi1pk` and `gsi1sk` for style-location queries
- **InstagramIndex (GSI2):** Uses `gsi2pk` for Instagram handle lookups (KEYS_ONLY projection for efficiency)

This configuration aligns with the Fargate task's GSI key generation algorithm to ensure optimal query performance.
...
The Fargate scraper tasks implement the following algorithm for generating Global Secondary Index keys:

**Style Processing Algorithm:**
...
FOR each artist profile:
  ...
  4. Validate against known style list: ['neotraditional', 'blackwork', 'realism', 'traditional', 'japanese', 'geometric', 'dotwork', 'watercolor', 'minimalism', 'tribal', 'illustrative', 'lettering', 'sketch', 'floral', 'anime', 'fineline', 'newschool']
  5. FOR each valid style:
       CREATE GSI1 entry with:
         gsi1pk = "STYLE#" + style
         gsi1sk = "LOCATION#" + country + "#" + city + "#" + artistId
```
...
```
FOR each artist profile:
  1. Extract location from studio information
  2. Normalize country: convert to uppercase ISO code (e.g., "UK", "US")
  3. Normalize city: convert to uppercase, remove special characters
  4. Validate location exists in approved geographic database
  5. Format as: "LOCATION#" + country + "#" + city + "#" + artistId
```

**Instagram Handle Processing Algorithm:**

```
FOR each artist profile:
  1. Extract Instagram handle from profile URL or bio
  2. Remove '@' symbol if present
  3. Convert to lowercase
  4. Validate handle format (alphanumeric + underscores only)
  5. Create GSI2 entry with:
       gsi2pk = "INSTAGRAM#" + handle
```
