...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '../../design-system/components/ui/Tooltip/Tooltip';
    ...
    <div className="w-full max-w-4xl mx-auto p-4" data-testid="style-filter">
      ...
      <div className="max-w-3/4">
        <div className="grid row-auto sm:grid-cols-4 md:grid-cols-5 gap-y-1 mb-1">
          {STYLES.map((style) => {
            const styleData = enhancedTattooStyles[style] || { name: style, description: style };
            const isSelected = selected.includes(style);            
            return (
              <TooltipProvider key={style} delayDuration={200}>
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs text-center">{styleData.description}</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            );
          })}
