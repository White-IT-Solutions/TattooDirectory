...
import { filterDOMProps } from "../../../utils/design-system-utils";
  ...
  (
    ...
    const buttonContent = (
      ...
      >
        ...
        {loading && children && <span className="ml-2">{children}</span>}
        {!loading && children}
      </button>
    );

    // Wrap with micro-feedback if enabled and not disabled
    if (withFeedback && !isDisabled) {
      return (
        <ButtonFeedback variant={feedbackType}>{buttonContent}</ButtonFeedback>
      );
    }
