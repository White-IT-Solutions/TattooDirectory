# =============================================================================
# APP SECURITY MODULE VARIABLES
# =============================================================================

variable "context" {
  description = "A single object containing all shared configuration values."
  type = object({
    # Core Identifiers
    project_name = string
    environment  = string
    name_prefix  = string

    # AWS Specific
    aws_region          = string
    replica_aws_region  = string
    account_id          = string # Infrastructure account
    security_account_id = string # Security account
    audit_account_id    = string

    # Common Configuration & Metadata
    common_tags                     = map(string)
    notification_email              = string
    allowed_countries               = list(string)
    enable_cross_region_replication = bool
    enable_advanced_monitoring      = bool
    log_retention_days              = number
    domain_name                     = string
    lambda_environment_vars         = map(string)
  })
  nullable = false
}

# Module-specific variables
variable "kms_key_main_arn" {
  description = "ARN of the main KMS key for encryption"
  type        = string
}

variable "opensearch_master_password" {
  description = "Password for OpenSearch master user"
  type        = string
  sensitive   = true
}

variable "app_secrets_password" {
  description = "Password for application secrets"
  type        = string
  sensitive   = true
}

variable "waf_firehose_arn" {
  description = "ARN of the WAF Kinesis Firehose stream in the Security Account"
  type        = string
}