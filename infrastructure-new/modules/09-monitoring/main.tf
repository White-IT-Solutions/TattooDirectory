...
# MONITORING MODULE
# =============================================================================
# This module contains monitoring and observability resources:
...
locals {
  # Log groups with standard retention
  log_groups_standard = {
    lambda_api       = "/aws/lambda/${var.context.name_prefix}-api-handler"
    lambda_sync      = "/aws/lambda/${var.context.name_prefix}-dynamodb-sync"
    lambda_workflow  = "/aws/lambda/${var.context.name_prefix}-workflow"
    api_gateway      = "/aws/apigateway/${var.context.name_prefix}-api"
    fargate_scraper  = "/aws/ecs/${var.context.name_prefix}-scraper"
    ecs_cluster      = "/aws/ecs/${var.context.name_prefix}-cluster"
    opensearch_audit = "/aws/opensearch/domains/${var.context.name_prefix}-search/audit-logs"
    cloudtrail       = "/aws/cloudtrail/${var.context.name_prefix}"
    step_functions   = "/aws/vendedlogs/states/${var.context.name_prefix}-data-aggregation"
  }

  # Log groups for operational tasks with longer retention
  log_groups_operational = {
    secret_rotation = "/aws/lambda/${var.context.name_prefix}-secret-rotation"
    rotate_eip      = "/aws/lambda/${var.context.name_prefix}-rotate-eip"
  }
}

# Standard retention log groups
resource "aws_cloudwatch_log_group" "standard" {
  for_each = local.log_groups_standard
  ...
  tags = merge(var.context.common_tags, {
    Name = "${var.context.name_prefix}-${each.key}-logs"
  })
}

# Operational log groups with longer retention
resource "aws_cloudwatch_log_group" "operational" {
  for_each = local.log_groups_operational
