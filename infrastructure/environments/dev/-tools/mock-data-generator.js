...
class MockDataGenerator {
    constructor() {
        ...
        this.studioNames = [
            'Ink & Steel', 'Sacred Art Tattoo', 'Black Rose Studio', 'Iron Wolf Tattoo',
            'Crimson Needle', 'Electric Lotus', 'Midnight Canvas', 'Golden Anchor',
            'Rebel Heart Tattoo', 'Mystic Ink', 'Urban Legend', 'Wild Spirit Tattoo',
            'Neon Dreams', 'Classic Lines', 'Modern Primitive', 'Artisan Ink'
        ];

        this.tattooStyles = [
            'traditional', 'neo-traditional', 'realism', 'blackwork', 'geometric',
            'watercolor', 'minimalist', 'japanese', 'tribal', 'biomechanical',
            'portrait', 'abstract', 'dotwork', 'linework', 'surrealism'
        ];
        ...
        this.portfolioDescriptions = [
            'Traditional rose with thorns', 'Realistic portrait piece', 'Geometric mandala design',
            'Japanese dragon sleeve', 'Minimalist line art', 'Watercolor butterfly',
            'Blackwork tribal pattern', 'Neo-traditional skull', 'Abstract geometric shapes',
            'Dotwork mountain landscape', 'Biomechanical arm piece', 'Surreal eye design'
        ];
    }
    ...
    generateArtist(options = {}) {
        ...
        const styles = options.styles || this.randomChoices(this.tattooStyles, 2, 4);
        const studioName = options.studioName || this.randomChoice(this.studioNames);

        const instagramHandle = name.toLowerCase().replace(/\s+/g, '') + 'tattoo';
        const website = `https://${name.toLowerCase().replace(/\s+/g, '')}.tattoo.com`;
        const email = `${name.toLowerCase().replace(/\s+/g, '.')}@${studioName.toLowerCase().replace(/\s+/g, '')}.com`;
        ...
        return {
            ...
            availability: this.randomChoice(['Available', 'Booking 2-3 weeks', 'Booking 1-2 months', 'Waitlist only']),
            specialties: this.randomChoices([
                'Custom designs', 'Cover-ups', 'Large pieces', 'Fine line work',
                'Color work', 'Black and grey', 'Walk-ins welcome'
            ], 1, 3),
    ...
    generatePortfolioImages(styles, count = null) {
        ...
        for (let i = 0; i < imageCount; i++) {
            ...
            images.push({
                ...
                style,
                tags: this.randomChoices(['healed', 'fresh', 'color', 'black-and-grey', 'large', 'small'], 1, 3),
    ...
    generateArtists(count = 20) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            artists.push(this.generateArtist({
                ...
                errors: [
                    {
                        field: 'style',
                        code: 'invalid_value',
                        message: 'Style must be one of: traditional, neo-traditional, realism, blackwork, geometric'
                    }
                ]
            },
            not_found: {
    ...
    generatePerformanceTestData(options = {}) {
        ...
        for (let i = 0; i < searchQueries; i++) {
            data.searchQueries.push({
                ...
                location: this.randomChoice([null, ...this.locations.map(l => l.city)]),
    ...
    async generateTestDataset(options = {}) {
        ...
        const dataset = {
            ...
            searchScenarios: {
                ...
                styleFilter: this.generateSearchResults({ style: 'traditional', limit: 5 }),
                locationFilter: this.generateSearchResults({ location: 'London', limit: 8 })
            },
            errorScenarios: {
                validation: this.generateErrorResponse('validation'),
                notFound: this.generateErrorResponse('not_found', { artistId: 'artist-999' }),
        ...
        if (saveFiles) {
            ...
            await this.saveToFile(dataset.searchScenarios, 'mock-search-scenarios.json');
            await this.saveToFile(dataset.errorScenarios, 'mock-error-scenarios.json');
            await this.saveToFile(dataset.performanceData, 'mock-performance-data.json');
        ...
        console.log(`📊 Total size: ${Math.round(dataset.metadata.totalSize / 1024)}KB`);
...
if (require.main === module) {
    ...
    switch (command) {
        ...
        case 'dataset':
            generator.generateTestDataset(options)
                .then(() => process.exit(0))
                .catch(error => {
...
Usage: node dev-tools/mock-data-generator.js <command> [options]
...
Examples:
  node dev-tools/mock-data-generator.js artists --count 10
  node dev-tools/mock-data-generator.js search --style traditional --limit 5
