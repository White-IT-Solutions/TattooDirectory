...
locals {
  ...
  # This truncates the prefix to ensure the final name is valid.
  # Max prefix length = 28 (total) - 7 ("-search") = 21
  opensearch_domain_name = "${substr(var.context.name_prefix, 0, 21)}-search"
}
    ...
    Statement = [
      {
        ...
        Action   = "es:*"
        Resource = "arn:aws:es:${var.context.aws_region}:${var.context.infra_account_id}:domain/${local.opensearch_domain_name}/*"
      }
    ]
  })
