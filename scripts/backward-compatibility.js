...
class BackwardCompatibilityLayer {
  ...
  async legacySelectiveSeeder(scenario) {
    this.showDeprecationWarning(
      ...
      `npm run seed --workspace=scripts-scenario ${scenario}`,
  ...
  checkLegacyScript(scriptPath) {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      if (scriptPath.includes('seed.js')) {
        ...
        console.log('   → Migrate to: npm run seed --workspace=scripts-scenario <scenario-name>');
        ...
        console.log('   → Migrate to: npm run validate --workspace=scripts/documentation-analysis-data');
      } else {
