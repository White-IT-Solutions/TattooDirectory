...
const utils = {
  ...
  async resetLocalStack() {
    ...
    try {
      ...
      execSync('docker-compose -f dev-tools/docker/docker-compose.local.yml restart localstack', { stdio: 'inherit' });
      
      console.log('⏳ Waiting for LocalStack to initialize...');
      await new Promise(resolve => setTimeout(resolve, 10000));
      
      console.log('🌱 Re-seeding test data...');
      execSync('docker-compose -f dev-tools/docker/docker-compose.local.yml run --rm data-seeder', { stdio: 'inherit' });
