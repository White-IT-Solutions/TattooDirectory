...
const REFERENCE_FIXES = {
  ...
  'docs/localstack/': 'docs/consolidated/development/',
  'docs/planning/': 'docs/consolidated/deployment/',
  'docs/scripts/': 'docs/consolidated/development/scripts/',
  'docs/frontend/': 'docs/consolidated/development/frontend/',
  'docs/backend/': 'docs/consolidated/development/backend/',
  'docs/architecture/': 'docs/consolidated/architecture/',
  'docs/diagrams_as_code/': 'docs/consolidated/architecture/diagrams/source/',
  'docs/devtools/': 'docs/consolidated/development/scripts/',
  'docs/reference/': 'docs/consolidated/reference/',
  'docs/troubleshooting/': 'docs/consolidated/troubleshooting/'
};
  ...
  try {
    ...
    for (const fileInfo of filesToFix) {
      try {
        ...
        const action = typeof fileInfo === 'object' ? fileInfo.action : 'general-fix';
        ...
        if (result.commandsFixed > 0 || result.referencesFixed > 0) {
          console.log(`  ✅ Fixed ${result.commandsFixed} commands, ${result.referencesFixed} references`);
        ...
        console.error(`  ❌ Error processing ${typeof fileInfo === 'string' ? fileInfo : fileInfo.path}:`, error.message);
...
async function getFilesToFix() {
  ...
  try {
    ...
    console.log(`📊 Found ${outdatedFiles.length} files from gap analysis report`);
  ...
  // Fallback to common files if gap analysis report is not available
  const commonFiles = [
    'README.md',
    'docs/README.md',
    'docs/QUICK_START.md',
...
async function findFileInProject(filename) {
  // Common locations to search for documentation files
  const searchPaths = [
    filename, // Root level
    `docs/${filename}`,
    ...
    `docs/consolidated/getting-started/${filename}`,
    `docs/consolidated/development/${filename}`,
    `docs/consolidated/deployment/${filename}`,
    `docs/consolidated/architecture/${filename}`,
    `docs/consolidated/reference/${filename}`,
    `docs/consolidated/troubleshooting/${filename}`
  ];
...
async function handleConsolidateAction(filePath, content, issues) {
  ...
  if (content.includes('⚠️ **DEPRECATED**') || content.includes('**This file has been moved**')) {
...
async function handleFixReferencesAction(content, issues) {
  ...
  for (const issue of issues) {
    ...
    if (brokenRefMatch) {
      ...
      // Fix additional common broken patterns
      const additionalFixes = {
        ...
        'docs/localstack/README_LOCAL.md': 'docs/consolidated/development/localstack.md',
...
async function handleUpdateCommandsAction(content, issues) {
  ...
  for (const issue of issues) {
    const commandMatch = issue.match(/Contains (\d+) outdated command references/);
    if (commandMatch) {
      ...
      for (const [oldCommand, newCommand] of Object.entries(additionalCommandFixes)) {
        const regex = new RegExp(escapeRegExp(oldCommand), 'g');
        const matches = content.match(regex);
        if (matches) {
          content = content.replace(regex, newCommand);
...
async function fixMarkdownLinks(content, filePath) {
  ...
  while ((match = linkRegex.exec(content)) !== null) {
    ...
    const absolutePath = path.resolve(path.dirname(path.join(projectRoot, filePath)), linkPath);
    try {
...
async function findReplacementPath(brokenPath) {
  ...
  // Common file relocations
  const relocations = {
    'README.md': 'docs/consolidated/README.md',
    'QUICK_START.md': 'docs/consolidated/getting-started/README.md',
    'local-development.md': 'docs/consolidated/development/local-setup.md',
    'docker-setup.md': 'docs/consolidated/getting-started/docker-setup.md',
    'dependencies.md': 'docs/consolidated/getting-started/dependencies.md',
    'command-reference.md': 'docs/consolidated/reference/commands.md',
    'api-documentation.md': 'docs/consolidated/reference/api-documentation.md',
    'system-overview.md': 'docs/consolidated/architecture/system-overview.md',
    'data-models.md': 'docs/consolidated/architecture/data-models.md',
    'api-design.md': 'docs/consolidated/architecture/api-design.md'
  };
