...
function getDockerComposeFiles() {
  const baseFile = 'dev-tools/docker/docker-compose.local.yml';
  const files = [baseFile];
  
  // Add platform-specific override
  if (isWindows) {
    files.push('dev-tools/docker/docker-compose.windows.yml');
  } else if (isMacOS) {
    files.push('dev-tools/docker/docker-compose.macos.yml');
  } else if (isLinux) {
    files.push('dev-tools/docker/docker-compose.linux.yml');
  }
  
  // Add general override if it exists
  if (fs.existsSync('dev-tools/docker/docker-compose.override.yml')) {
    files.push('dev-tools/docker/docker-compose.override.yml');
  }
...
async function startEnvironment() {
  ...
  logInfo(`Memory: ${platformInfo.freeMemory} free of ${platformInfo.totalMemory}`);
  ...
  const requiredFiles = ['dev-tools/docker/docker-compose.local.yml'];
...
async function stopEnvironment(options = {}) {
  ...
  if (options.volumes) {
    logWarning('All data has been removed. Next startup will create fresh data.');
  }
  if (options.images) {
    logWarning('Images have been removed. Next startup will rebuild containers.');
  }
}
