...
docker-compose -f dev-tools/docker/docker-compose.local.yml down --remove-orphans > /dev/null 2>&1 || true
...
if docker-compose -f dev-tools/docker/docker-compose.local.yml ps | grep -q "data-seeder"; then
