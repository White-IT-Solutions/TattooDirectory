  ...
  static getDockerComposeCommand() {
    try {
      ...
      if (this.commandExists('docker')) {
        const result = this.executeCommandSync('docker compose version');
        if (result.success) {
...
if (require.main === module) {
  // Wrap in async function to handle any potential async operations
  (async function() {
    try {
      ...
      console.error('❌ Error in platform utilities:', error.message);
      process.exit(1);
    }
  })();
}