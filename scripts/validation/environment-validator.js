...
class EnvironmentValidator {
    ...
    async validateDockerEnvironment() {
        ...
        // Check if devtools/docker/docker-compose.local.yml exists
        try {
            await fs.access('devtools/docker/docker-compose.local.yml');
            dockerValidation.composeFileExists = true;
            console.log('  ✅ devtools/docker/docker-compose.local.yml exists');
        } catch (error) {
            dockerValidation.composeFileExists = false;
            console.log('  ❌ devtools/docker/docker-compose.local.yml not found');
        }
    ...
    async checkServiceRunning(serviceName) {
        try {
            const output = execSync(`docker-compose -f devtools/docker/docker-compose.local.yml ps ${serviceName} --format json`, { encoding: 'utf8' });
    ...
    async validateDockerComposeConfig() {
        ...
        try {
            const composeContent = await fs.readFile('devtools/docker/docker-compose.local.yml', 'utf8');
